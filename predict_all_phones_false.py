...
if __name__ == '__main__':
    all_data_dir = './FALSE_dataset_all_phones_cut'
    model_dir = './pbtxt'
    ...
    for phone_dir in phones_dir:
        ...
        for label_dir in labels_dir:
            model_num_dir = label_dir.split('_')[-1]
            if model_num_dir not in labels:
                no_label.append(label_dir)
                continue

            model_name = 'model_MI8_iphone7_' + model_num_dir
    ...
    print(csv_data)
    print(no_label)
    with open('./predict_accuracy_false.csv', 'a+', newline='', ) as csvfile:
