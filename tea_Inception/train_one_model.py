...
# 保存训练数据通过瓶颈层后提取的特征向量
CACHE_DIR = './inception_dec_2015/tmp/bottleneck_paper_dataset_longjingex'
# 图片数据的文件夹
INPUT_DATA = './paper_dataset/longjing'
...
def add_final_training_ops(class_count, bottleneck_tensor):
    # 输入
    bottleneck_input = tf.compat.v1.placeholder_with_default(bottleneck_tensor, [None, BOTTLENECK_TENSOR_SIZE],
                                                   name='BottleneckInputPlaceholder')
    ground_truth_input = tf.compat.v1.placeholder(tf.float32, [None, class_count], name='GroundTruthInput')
    # 全连接层
    with tf.name_scope('output'):
        weights = tf.Variable(tf.compat.v1.truncated_normal([BOTTLENECK_TENSOR_SIZE, class_count], stddev=0.001))
...
def train():
    ...
    print(bottleneck_tensor.graph is tf.compat.v1.get_default_graph())
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        ...
        with tf.compat.v1.gfile.FastGFile('./pbtxt/nn.pb', mode='wb') as f:
