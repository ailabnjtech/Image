...
# 保存训练数据通过瓶颈层后提取的特征向量
# CACHE_DIR = './inception_dec_2015/tmp/bottleneck_MI8_PENG'
# 图片数据的文件夹
# INPUT_DATA = './inception_dec_2015/MI8_iphone7'
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    if not os.path.exists(bottleneck_path):  # 如果不存在
        ...
        with open(bottleneck_path, 'r') as bottleneck_file:
            bottleneck_string = bottleneck_file.read()  # 字符串转float数组
...
def train(model_path):
    ...
    print(image_lists)
    # print(len(image_lists['daisy']['training']))
    # print(len(image_lists['daisy']['testing']))
    # print(len(image_lists['daisy']['validation']))
    ...
    with tf.Session(graph=graph) as sess:
        ...
        model_row = model_path.split('/')[-1]
        accuracy_row = '%.1f%%' % (test_accuracy * 100)
        csv_data[count].append(model_row)
        csv_data[count].append(accuracy_row)
...
if __name__ == '__main__':
    all_data = './inception_dec_2015/data_01_train'
    all_data_bottleneck = './inception_dec_2015/tmp/data_01_train'
    labels = os.listdir(all_data)
    csv_data = [[]for i in range(len(labels))]
    count = 0
    for label in labels:
        INPUT_DATA = os.path.join(all_data, label)
        CACHE_DIR = os.path.join(all_data_bottleneck, 'bottleneck_' + label)
        model_path = './pbtxt/model_' + label + '.pb'
        train(model_path)
        count += 1
    with open('./pbtxt/accuracy.csv', 'w', newline='',) as csvfile:
